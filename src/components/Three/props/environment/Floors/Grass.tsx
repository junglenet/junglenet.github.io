...
export default function Grass({ options = { bW: 0.12, bH: 1, joints: 5 }, width = 100, instances = 50000, ...props }) {
  ...
  const [texture, alphaMap] = useLoader(THREE.TextureLoader, ['textures/blade_alpha.jpg', '/textures/blade_diffuser.jpg'])
  ...
  // @ts-ignore
  useFrame((state) => materialRef?.current && (materialRef.current.uniforms.time.value = state.clock.elapsedTime / 4))
